...
public class ConfigurationFunctionExamples37 {
	...
	public void method1() {
		...
		String	namelist  = JsonPath.using(config).parse(json).read("$.Students[1].Name"); //1) Null  Option  : when using definite path but key not found in JSON at the time Path not found exception will get we can use DEFAULT_PATH_LEAF_TO_NULL method overcome
		List<Object>	namelist2 = JsonPath.using(config).parse(json).read("$.Students[*].Name");
		System.out.println(namelist); // output : null
		System.out.println(namelist2); // output : ["Arun",null]
	...
	public void method2() {
		...
		System.out.println(namelist); // output : ["Arun"]
	} 
